# ONDRSTRM App — Setup & Deploy Instructies
...
2. Klik **"Project toevoegen"** → noem het `ondrstrm-app`
...
# Configureer (in splintr_app/ map)
flutterfire configure --project=ondrstrm-app
```
...
5. Maak een Stripe webhook aan:
   - Endpoint: `https://europe-west1-ondrstrm-app.cloudfunctions.net/stripeWebhook`
...
# 2. Stel in Xcode:
#    - Bundle Identifier: nl.ondrstrm.splintr
...
```bash
keytool -genkey -v -keystore ondrstrm-upload-key.jks \
...
keyAlias=ondrstrm-key
storeFile=../../ondrstrm-upload-key.jks
```
...
2. Maak een nieuwe app "ONDRSTRM"
...
stripe listen --forward-to localhost:5001/ondrstrm-app/europe-west1/stripeWebhook
```
